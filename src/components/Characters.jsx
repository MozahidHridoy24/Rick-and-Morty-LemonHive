...
export default function Characters() {
  ...
  return (
    <div className="px-8 py-12 relative">
      <h2 className="text-2xl text-white font-bold mb-6">Meet The Cast</h2>

      {/* Prev button */}
      {canScrollLeft && (
        <button
          onClick={() => scrollBy("prev")}
          className="absolute left-2 top-1/2 -translate-y-1/2 bg-white bg-opacity-70 p-2 rounded-full z-10 hover:bg-opacity-90"
          aria-label="Previous"
        >
        ...
        {characters.map((char) => (
          ...
          >
            <div className="bg-gray-800 w-full h-64 rounded-lg p-4 flex flex-col items-center">
              ...
              />
              <h3 className="w-full text-start font-semibold text-white ">
                {char.name}
              </h3>
            </div>
          </div>
      ...
      {canScrollRight && (
        <button
          ...
          className="absolute right-2 top-1/2 -translate-y-1/2 bg-white bg-opacity-70 p-2 rounded-full z-10 hover:bg-opacity-90"
          aria-label="Next"
        >
